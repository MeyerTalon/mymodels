...
- **Manage dependencies with uv.** Treat `pyproject.toml` as the dependency declaration and commit `uv.lock`. Use `uv add <package>` for runtime dependencies, `uv add --dev <package>` for development dependencies, and `uv remove` to remove them. Use `uv sync` to create or update `.venv`, and run Python commands through `uv run`; don't use pip, conda, or manually maintained requirements files.
- **Unit tests for critical functions.** When writing or changing a function whose failure would break training, inference, or data handling, add a lightweight unit test that confirms its core behavior. Tests must run in seconds: tiny inputs, tiny model dims, no network, no downloads, no training loops, no real weights. Use `pytest` through `uv run pytest` (plain test functions with asserts — no test classes unless grouping helps). Put tests in `<pkg>/tests/test_<module>.py`. Skip tests for trivial glue or one-off scripts.
